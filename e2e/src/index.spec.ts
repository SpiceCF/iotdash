...
describe("DefaultApi Test Suite", () => {
  ...
  it("should have sensor metric logs", async () => {
    const yesterday = formatRFC3339(addDays(new Date(), -1));
    const tomorrow = formatRFC3339(addDays(new Date(), 1));

    const getSensorMetricLogsRes = await sensorAPI.getSensorsIdLogsMetric({
      id: sensorID,
      key: "temperature",
      from: yesterday,
      to: tomorrow,
    ...
    expect(getSensorMetricLogsRes.data?.avg?.length).toBeGreaterThan(0);
    expect(getSensorMetricLogsRes.data?.max?.length).toBeGreaterThan(0);
    expect(getSensorMetricLogsRes.data?.min?.length).toBeGreaterThan(0);
  });
});
